...
&#8195; This is a vue-cli 3.x plugin for webpack DllPlugin/DllReferencePlugin that can drastically improve build time performance.`vue-cli-plugin-dll` register `dll` instruction to avoid adding extra webpack configuration file, it also insert DllReferencePlugin and inject chunk files automatically when you run `dev/build` directives.
